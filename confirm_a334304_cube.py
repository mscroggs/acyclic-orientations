...
def calculate_term_check(dim, printing=False):
    # generate the edges of a dim-dimensional cube
    if dim <= 1:
        return 1
    topology = {}
    ...
    for n in permutations(range(2**dim)):
        o = "".join(["1" if n[i[0]] > n[i[1]] else "0" for i in edges])
        for p in transforms:
            o2 = "".join(["1" if n[p[i[0]]] > n[p[i[1]]] else "0" for i in edges])
            if o2 in found:
                break
        else:
            found.append(o)
...
a_3 = calculate_term_check(3)
print(a_3)
assert a_3 == calculate_term(3)
